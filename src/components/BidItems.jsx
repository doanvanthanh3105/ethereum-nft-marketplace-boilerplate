...
function BidItems() {
  ...
  async function riseBid() {
    const p = price * ('1e' + 18);

    setLoading(true);
    const ops = {
      contractAddress: auctionAddress,
      functionName: riseBidFunction,
  ...
  async function updateBidItem() {
    ...
    await query.get(id).then((obj) => {
      console.log(obj);
      obj.set('currentBidder', walletAddress);
      obj.set('currentPrice', String(price * ('1e' + 18)));
      obj.save();
    });
  }
  ...
  function succPurchase() {
    ...
    const modal = Modal.success({
      title: 'Success!',
      content: `You have raise this NFT`,
    });
    setTimeout(() => {
      modal.destroy();
    }, secondsToGo * 1000);
  ...
  function failPurchase() {
    ...
    const modal = Modal.error({
      title: 'Error!',
      content: `There was a problem when raising this NFT`,
    });
    setTimeout(() => {
      modal.destroy();
    }, secondsToGo * 1000);
    ...
    <div style={styles.NFTs}>
      {fetchBidItems?.slice(0, 20).map((nft, index) => (
        <Card
          ...
          key={index}>
          {<Badge.Ribbon text='Bid Now' color='green'></Badge.Ribbon>}
          <Meta
            title={getPrice(nft) / ('1e' + 18) + '  MATIC'}
            description={`${getName(nft.nftContract, nft.tokenId)} `}
          />
          <Meta
            description={`current bidder: ${getEllipsisTxt(
              nft.currentBidder,
              3
            )}`}
          />
      ...
      <Modal
        ...
        <Spin spinning={loading}>
          ...
          <Input
            autoFocus
            placeholder='Rising Price for bidding'
