...
function Favorites() {
  ...
  const fetchFavorites = JSON.parse(
    ...
    (item) =>
      item.walletAddress === walletAddress && item.currentFavorite === true
  );
  ...
  const handleFavoriteClick = async (nft) => {
    const query = new Moralis.Query(Favorite);
    await query.get(nft.objectId).then((obj) => {
  ...
  return (
    <div style={styles.NFTs}>
      {fetchFavorites.slice(0, 20).map((nft, index) => (
        <Card
          onClick={() => {
            console.log(nft);
          }}
      ...
      {getMarketItem(nftToBuy) ? (
        <Modal
          title={`Buy ${nftToBuy?.name} #${nftToBuy?.tokenId}`}
          visible={visible}
